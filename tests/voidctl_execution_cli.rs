...
#[test]
fn interactive_yolo_dry_run_prints_error_for_bridge_failure() {
    let (base_url, requests, server) = spawn_fake_bridge(vec![FakeResponse {
        status: 400,
        body: json!({
            "message": "yolo spec must include at least one job"
        }),
    }]);

    let inputs_path = temp_inputs_path("yolo-invalid.json");
    fs::write(
        &inputs_path,
        r#"{"api_version":"v1","kind":"yolo","worker":{"template":"examples/runtime-templates/warm_agent_basic.yaml"},"jobs":[]}"#,
    )
    .expect("write inputs");
    ...
    let mut child = voidctl_command(&base_url)
        ...
        .spawn()
        .expect("spawn voidctl");
    let command = format!("/yolo dry-run {}\n/exit\n", inputs_path.display());
    child
        .stdin
        .take()
        .expect("stdin")
    ...
    assert!(output.status.success());
    let stdout = String::from_utf8(output.stdout).expect("stdout");
    assert!(stdout.contains("error: yolo spec must include at least one job"));

    let requests = requests.lock().expect("lock requests");
    assert_eq!(requests.len(), 1);
    assert_eq!(requests[0].method, "POST");
    assert_eq!(requests[0].path, "/v1/yolo/dry-run");
}
